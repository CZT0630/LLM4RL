...
from algos.mappo.mappo_agent import MAPPOAgent
...
def train_mappo(config=None):
    if config is None:
        config = load_config()
    set_seed(config.get('seed', 42))

    path_manager = get_path_manager()
    model_dir = path_manager.get_model_path("mappo")
    ...
    max_episodes = mappo_cfg.get('max_episodes', 200)
    max_steps = mappo_cfg.get('max_steps', 200)
    ...
    for episode in range(max_episodes):
        ...
        buffer.compute_advantages(gamma=mappo_cfg['gamma'], lam=mappo_cfg['lam'])
        step_losses = []
        for i, agent in enumerate(agents):
    ...
    if training_losses:
        save_training_metrics_csv(training_losses, os.path.join(data_dir, 'mappo_training_metrics.csv'))
    plotter.plot_training_curves(training_losses, algo_name='MAPPO')
